...
// ---------------------------------------------------------------------------
// Buyer-side guard for the official x402 client (@x402/core) hook
// onBeforePaymentCreation. The seller-side middleware above screens who is
// PAYING you; this screens who you are ABOUT TO PAY, before any signature
// is created.
//
// Hook contract (from @x402/core): (declaration, context) =>
//   Promise<void | { abort: true; reason: string }>, where
//   context.selectedRequirements.payTo is the recipient.
//
// Usage:
//   client.onBeforePaymentCreation(rubricBuyerGuard());
//
// Composed with your own spend policy:
//   const screen = rubricBuyerGuard();
//   client.onBeforePaymentCreation(async (decl, ctx) => {
//     const s = await screen(decl, ctx);
//     if (s) return s;
//     // ...your network/asset/cap checks...
//   });

export function rubricBuyerGuard(opts: {
  /** Abort when the sanctions list cannot be loaded. Default false (fail
   *  open), matching the seller middleware. Set true if unscreened spend is
   *  worse for you than a blocked request. Either way the screening result
   *  discloses listUnavailable, so the decision is on the record. */
  abortOnUnavailable?: boolean;
  /** Called with every screening result - wire this to your agent's log so
   *  each allow/abort decision leaves a record. */
  onScreened?: (result: ScreenResult, payTo: string) => void;
  listUrl?: string;
  maxListAgeMs?: number;
} = {}) {
  return async function (
    _declaration: unknown,
    context: { selectedRequirements?: { payTo?: string } },
  ): Promise<void | { abort: true; reason: string }> {
    const payTo = context?.selectedRequirements?.payTo ?? null;
    if (!payTo || !/^0x[a-fA-F0-9]{40}$/.test(String(payTo))) {
      // No EVM payTo to screen (different rail, or malformed challenge).
      // Not this guard's call - pass through to the caller's policy.
      return;
    }
    const result = await screenPayer(String(payTo).toLowerCase(), {
      ...(opts.listUrl ? { listUrl: opts.listUrl } : {}),
      ...(opts.maxListAgeMs ? { maxListAgeMs: opts.maxListAgeMs } : {}),
    });
    try { opts.onScreened?.(result, String(payTo)); } catch {}
    if (result.ofacMatch) {
      return { abort: true,
    ...
    return; // clear (or fail-open): payment proceeds
  };
}
